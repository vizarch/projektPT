...
class Command(BaseCommand):
    help = 'Scrap altcontroldelete.pl'
    ...
    def handle(self, *args, **options):
        ...
        new_source = Sources(name="altcontroldelete.pl")
        try:
            new_source.save()
        except IntegrityError:
            # Source exists, so find it
            new_source = Sources.objects.get(name="altcontroldelete.pl")

        # run thread for scraper
        start = threading.Thread(target=altcontroldelete.scrapshot, args=(pages,))
        start.start()

        # while queue has item or while END flag is set to False
        while not altcontroldelete.ARTICLES.empty() or not altcontroldelete.END:
            one_art = altcontroldelete.ARTICLES.get()
